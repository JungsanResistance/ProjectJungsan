  ...
  transaction: (from, to, type) => {
    ...
    const sendData = {
      ...
      message: `<p><b>${from.username}</b>님께서 정산 확인을 요청하였습니다!</p>
              <p><a href="http://localhost:3000">프로젝트 정산 홈페이지</a>에서 내역을 확인하신 후, 정산 수락 혹은 거절을 선택해주세요.</p>
    ...
    if (type === 'accept') {
      ...
      sendData.message = `<p><b>${from.username}</b>님께서 정산 요청을 수락하였습니다!</p>
              <p><a href="http://localhost:3000">프로젝트 정산 홈페이지</a>에서 내역을 확인해주세요.</p>
              <p>저희 서비스를 이용해주셔서 감사합니다.</p>
              <br>
              <br>
              <p>프로젝트 정산 관리자 드림</p>
    ...
    if (type === 'reject') {
      ...
      sendData.message = `<p><b>${from.username}</b>님께서 정산 요청을 거절하였습니다.</p>
              <p><a href="http://localhost:3000">프로젝트 정산 홈페이지</a>
  ...
  duplicate: (profile) => {
    ...
    const sendData = {
      ...
      message: `
              ...
              <p><a href="http://localhost:3000/auth/${source}">여기를 클릭</a>하셔서 해당 계정으로 로그인해주세요.</p>
              <p>저희 서비스를 이용해주셔서 감사합니다.</p>
              <br>
              <br>
              <p>프로젝트 정산 관리자 드림</p>
