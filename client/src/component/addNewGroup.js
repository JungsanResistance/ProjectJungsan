...
export default class AddNewGroup extends React.Component {
  ...
  componentWillMount() {
    axios.get('http://http://ec2-52-78-111-241.ap-northeast-2.compute.amazonaws.com/api/misc')
    .then((res) => {
      console.log(res);
  ...
  handleSubmit() {
    if (!this.state.groupname.length) {
      ...
      axios.post('http://http://ec2-52-78-111-241.ap-northeast-2.compute.amazonaws.com/api/group', {
  ...
  handleGroupName() {
    if (!this.state.groupname.length) {
      ...
      axios.get(`http://http://ec2-52-78-111-241.ap-northeast-2.compute.amazonaws.com/api/groupedit?target=groupname&groupname=${this.state.groupname}`)
      .then((res) => {
        const data = JSON.parse(res.data);
        if (data.length) {
          this.setState({
