...
export default class GroupEditForm extends React.Component {
  ...
  componentWillMount() {
    axios.get(`http://ec2-13-124-106-58.ap-northeast-2.compute.amazonaws.com/api/groupedit?target=groupmembers&groupname=${this.props.params.groupname}`)
    .then((res) => {
      console.log("res!!!!!!!!!", res)
      ...
      if (emptyGroupMembersCheck) {
        ...
        if ((groupNameCheck === false) && groupMemberCheck) {
          console.log('this is the data we are submiting', {
            ...
            axios.put(`http://ec2-13-124-106-58.ap-northeast-2.compute.amazonaws.com/api/group`,
              {
                  oldgroupname: this.state.oldGroupname,
                  newgroupname: this.state.newGroupname,
                  action: 'modifyGroupName',
  ...
  handleAddMember() {
    ...
    axios.get(`http://ec2-13-124-106-58.ap-northeast-2.compute.amazonaws.com/api/groupedit?target=email&email=${this.state.emailToBeChecked}`)
    .then((res) => {
  ...
  handleGroupName() {
    ...
    axios.get(`http://ec2-13-124-106-58.ap-northeast-2.compute.amazonaws.com/api/groupedit?target=groupname&groupname=${this.state.newGroupname}`)
    .then((res) => {
      const data = JSON.parse(res.data);
