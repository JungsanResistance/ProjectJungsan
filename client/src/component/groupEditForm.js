...
export default class GroupEditForm extends React.Component {
  ...
  componentWillMount() {
    axios.get(`http://ec2-52-78-111-241.ap-northeast-2.compute.amazonaws.com/api/groupedit?target=groupmembers&groupname=${this.props.params.groupname}`)
    .then((res) => {
      console.log("res!!!!!!!!!",res)
    ...
    this.state.errorGroupnameDuplicate !== '이 그룹이름은 이미 있어 띵구야') {
      ...
      if (emptyGroupMembersCheck) {
        ...
        if ((groupNameCheck === false) && groupMemberCheck) {
            axios.put(`http://ec2-52-78-111-241.ap-northeast-2.compute.amazonaws.com/api/groupedit`,
              {
                body: {
  ...
  handleGroupName() {
    axios.get(`http://ec2-52-78-111-241.ap-northeast-2.compute.amazonaws.com/api/groupedit?target=groupname&groupname=${this.state.newGroupname}`)
    .then((res) => {
      const data = JSON.parse(res.data);
