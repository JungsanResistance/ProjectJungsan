...
export default class GroupEditForm extends React.Component {
  ...
  componentWillMount() {
    ...
    .then((res) => {
      ...
      const groupMemberData = groupData.map((data) => {
        return {username: data.username, active: data.active};
      })
  ...
  handleMemberDelete(event) {
    const NextGroupmembers = [...this.state.groupMembers];
    NextGroupmembers.map((data, index) => {
  ...
  render() {
    ...
    this.state.groupMembers.forEach((data) => {
      if(data.active){
        members.push(<li>{data.username} <input type="submit" name={data.username} value="delete" onClick={this.handleMemberDelete}/></li>)
      }
    });
    ...
    return(
      <div>
        <h2>groupname :</h2>
          <input type="text" className="editGroupName" placeholder={this.props.params.groupname} onChange={this.handleGroup}/>
        <br />
        <br />
        <h2>groupmember :</h2>
          Add groupmember : <input type='text' className="editGroupMember" palceholder='ex) wnghee91@gmail.com' onKeyPress={this.handleKeyPress} onChange={this.handleGroup}/>
